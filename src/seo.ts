...
export const pages: Record<string, { title: string; description: string; label: string }> = {
  ...
  '/recursos/': { title: 'Blog de Quiebras en Puerto Rico | Lcda. Raquel Núñez', description: 'Guías sencillas sobre quiebras, deudas, embargos, Capítulo 7 y Capítulo 13 para Cidra y la zona central de Puerto Rico.', label: 'Recursos' },
...
export function seoHead(path: string) {
  ...
  if (path !== '/') {
    ...
    if (path.startsWith('/recursos/quiebras/')) crumbs.push({ name: 'Recursos', item: origin + '/recursos/' });
  ...
  if (path.startsWith('/recursos/quiebras/')) {
